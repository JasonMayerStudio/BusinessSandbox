...
class ReportDetailView extends Component {
  ...
  goBack = () => {
    ...
    this.props.history.push({
      pathname: `${this.props.detailDrawerBaseUrl}${reportId}`,
    });
  };
  ...
  gotoDetailView = (row) => {
    ...
    const reportId = state.reportId;
    this.props.history.push({
      pathname: `${this.props.detailDrawerBaseUrl}${reportId}/details/${row.seq_key}`,
      state: {
        row,
...
ReportDetailView.propTypes = {
  ...
  detailDrawerBaseUrl: PropTypes.string.isRequired,
};

export default withRouter(ReportDetailView);
