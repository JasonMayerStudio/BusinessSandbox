...
class Equipment extends Component {
  ...
  componentDidMount() {
    this.props.reportActions.getSingleReportData('equipment_paginated');
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.reportDefinition.primaryDateFilterId &&
      ...
      const newPrimaryDateFilter = {
        id: nextProps.reportDefinition.primaryDateFilterId,
        v1: 'last_seven_days',
        type: 'STARTS_WITH',
      };
    ...
    const filtersWithType = newFilterValues.map((filterValue) => {
      ...
      const isDateFilter =
        this.props.dateFilters.findIndex(
          (dateFilter) => dateFilter.reportFilterId === filterValue.id,
  ...
  goToDetailView = (row, detailViewStructure) => {
    ...
    this.props.history.push({
      pathname: `/locations/equipment/${reportId}/details/${row.seq_key}`,
      state: {
        row,
        detailViewStructure,
...
function mapStateToProps(state) {
  const report = state.reportsV2.data.find((item) => {
    return item.key.includes('equipment');
  });
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(Equipment),
);
