...
class ReportView extends Component {
  ...
  applyFilters(newValues) {
    ...
    const filtersWithType = newFilterValues.map((filterValue) => {
      ...
      // @todo, 2018-03-28 (vjw), use only the first array value of a mulitselect's values
      //                          --> temporarily needed until API has a way to handle multiple
      //                          v1 values
      const isMultiselectFilter =
        this.props.multiselectFilters.findIndex(
          (multiselectFilter) => multiselectFilter.reportFilterId === Number(filterValue.id),
        ) > -1; // eslint-disable-line
      if (isMultiselectFilter) {
        const tempSingleValue = filterValue.v1.shift();
        filterValue.v1 = tempSingleValue; // eslint-disable-line
      }
  ...
  handleRowAction({ row, action }) {
    console.log('in handleRowAction, row: ', row);
    console.log('in handleRowAction, action: ', action);
    ...
    console.log('compositeLinks: ', compositeLinks);
