...
class GDReportBuilder extends Component {
  constructor(props) {
    ...
    let activeColumns;
    if (props.selectedTemplateId && props.report.reportId) {
      const dataColumns = props.report.dataColumns ? props.report.dataColumns : [];
      const updatedAvailableColumns = this.getAvailableColumns(props.templates, dataColumns, props.report.templateId);
      availableColumns = updatedAvailableColumns.filter((column) => !column.isSelected && !column.required);
      activeColumns = updatedAvailableColumns.filter((column) => column.required || column.isSelected);
    }
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.selectedTemplateId !== this.props.selectedTemplateId) {
      const chosenTemplateId = nextProps.report.templateId || nextProps.selectedTemplateId;

      const dataColumns = this.state.report.dataColumns ? this.state.report.dataColumns : [];
      const updatedAvailableColumns = this.getAvailableColumns(nextProps.templates, dataColumns, chosenTemplateId);
    ...
    if (nextProps.report.reportId !== this.props.report.reportId) {
      ...
      const updatedAvailableColumns = this.getAvailableColumns(nextProps.templates, nextProps.report.dataColumns, nextProps.report.templateId);
      const availableColumns = updatedAvailableColumns.filter((column) => !column.isSelected && !column.required);
      const activeColumns = updatedAvailableColumns.filter((column) => column.required || column.isSelected);
  ...
  getAvailableColumns = (availableCols, activeCols, templateId) => {
    const columns = compose(
      (cols) => this.mergeActiveColumns(cols, activeCols),
      (cols) => mapInitialColumns(cols),
      (temps) => this.getTemplateColumns(temps, templateId),
    );
    return columns(availableCols);
  }

  getTemplateColumns = (availableCols, templateId) => {
    if (availableCols.length === 0) return [];
    const template = availableCols.find((item) => { return item.id === templateId; });
  ...
  openAccordion(currentTarget) {
    const firstToggleState = Object.keys(this.state.accordions).reduce((newObj, key) => {
      newObj[key] = (key === currentTarget) // eslint-disable-line no-param-reassign
