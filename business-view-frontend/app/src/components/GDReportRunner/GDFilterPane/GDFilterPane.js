...
class GDFilterPane extends Component {
  ...
  handleSelection = (value, event) => {
    ...
    if (foundFilter) {
      ...
      const selectedItemsIndex = (currentMultiSelectValue)
        ? currentMultiSelectValue.indexOf(value) : null;

      if (selectedItemsIndex !== -1) {
        currentMultiSelectValue.splice(selectedItemsIndex, 1);
      } else {
    ...
    return this.state.sortedFilterList.map((filter) => {
      return (
        <GDFilterElement
          ...
          id={`${this.props.reportName}_filterElement`}
        />
      );
    });
  }
