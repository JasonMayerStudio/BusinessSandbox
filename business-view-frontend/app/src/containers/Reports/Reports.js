...
export class Reports extends Component {
  ...
  get reportCards() {
    ...
    const parseCategories = categoryList.map((cat) => {
      ...
      const defaultReports = reports.filter((item) => {
        return (
          item.systemReport === true &&
          (
            item.reportName.toLowerCase().includes(search.toLowerCase()) ||
            item.reportDescription.toLowerCase().includes(search.toLowerCase())
          )
        );
      });

      const customReports = reports.filter((item) => {
        return (
          item.systemReport === false &&
          (
            item.reportName.toLowerCase().includes(search.toLowerCase()) ||
            item.reportDescription.toLowerCase().includes(search.toLowerCase())
          )
