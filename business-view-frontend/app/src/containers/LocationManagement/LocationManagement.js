...
class LocationManagement extends Component {
  ...
  componentDidMount() {
    this.props.reportActions.getSingleReportData('location_information_paginated');
  ...
  goToDetailView = (row, detailViewStructure) => {
    ...
    const isHeartlandUser = hasHeartlandHierarchyAccess(this.props.dataAccess);
    const backUrl = isHeartlandUser === true ? '/managelocations/' : '/locations/management/';
    this.props.history.push({
      pathname: `${backUrl}${reportId}/details/${row.seq_key}`,
      state: {
        row,
        detailViewStructure,
      ...
      <div className="location-view">
        ...
        {Boolean(this.props.reportDefinition.dataColumns.length) && (
          <GDReportRunner
            ...
            hasFilterAccess={this.props.hasFilterAccess}
            hasActionAccess={this.props.hasActionAccess}
          />
        )}
      </div>
    );
...
LocationManagement.propTypes = {
  ...
  dataAccess: PropTypes.array,
  hasFilterAccess: PropTypes.bool.isRequired,
  hasActionAccess: PropTypes.bool.isRequired,
};
...
function mapStateToProps(state) {
  const dataAccess = state.auth.session.user.dataAccess;
  const report = state.reportsV2.data.find((item) => {
    return item.key.includes('location_information_paginated');
  });
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(LocationManagement),
);
